...
import { handleOperations } from '../../../lib/operationsApi';
import { jsonResponse } from '../../../lib/http';

export const onRequest: PagesFunction<AppEnv> = async ({ request, env }) => {
  if (request.method === 'OPTIONS') {
    return asPagesResponse(await handleOperations(request as unknown as Request, env));
  }

  const user = await getAuthorizedUser(request, env);
  if (!user) return jsonResponse<AppEnv>({ error: 'Authentication required' }, 401);

  return asPagesResponse(await handleOperations(request as unknown as Request, env, user.id));
};
